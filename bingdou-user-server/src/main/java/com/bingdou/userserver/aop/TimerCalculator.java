package com.bingdou.userserver.aop;
...
public class TimerCalculator {
    ...
    @Pointcut("execution(* com.bingdou.live.userserver.controller.*.*(..))")
        ...
        try {
            ...
            if (StringUtils.isEmpty(requestMethod)) {
                ...
                if (requestMethod.contains("_"))
                    method = methodName + "/userserver-" + requestMethod.replaceAll("_", "-");
                else
                    method = methodName + "/userserver-" + requestMethod;
            }
        } catch (Throwable e) {
            throw e;
        } finally {
